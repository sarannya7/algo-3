...
app = Flask(__name__, template_folder='../algo-3')
...
def test(data1):
	...
	injson =  js.loads(data1)
	graph = Graph(amount_vertices=injson['size'])

	# This graph is in the folder "images" of the repository.
	
	for item in injson['data']:
		graph.addEdge(item['S'], item['D'],item['W'])
